...
public class PlayerInputOld : MonoBehaviour {
	...
	void Update () {
        ...
        {
            //PlayerMovement
            goPlayer.GetComponent<PlayerOld>().input = new Vector2(Input.GetAxisRaw("Horizontal"), Input.GetAxisRaw("Vertical"));

            if (Input.GetKeyDown(KeyCode.W))
                goPlayer.GetComponent<PlayerOld>().CanJump = true;
            if (Input.GetKeyUp(KeyCode.W))
                goPlayer.GetComponent<PlayerOld>().CanJump = false;
            //Shooting
            ...
            if (goPlayer.GetComponent<PlayerOld>().input.x != 0 || goPlayer.GetComponent<PlayerOld>().input.y != 0)
            {
                isMoving = true;
            }
            else
            ...
            if (!canInput)
            {
                goPlayer.GetComponent<PlayerOld>().input.x = 0;
                goPlayer.GetComponent<PlayerOld>().input.y = 0;
            }
        ...
        {
                //PlayerMovement
                goPlayer.GetComponent<PlayerOld>().input = new Vector2(XCI.GetAxisRaw(XboxAxis.LeftStickX), XCI.GetAxisRaw(XboxAxis.LeftStickY));
